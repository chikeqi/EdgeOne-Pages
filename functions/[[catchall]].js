...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .header-content h1{color:#d4f5e2;text-shadow:0 2px 12px rgba(0,0,0,0.6),0 0 30px rgba(100,255,180,0.4);display:inline-block;margin-right:20px}
...
<body>
    ...
    <div class="main">
        <div class="header" id="siteHeader">${headerBg ? `<img class="header-bg-img" id="headerBgImg" src="${escapeHtml(headerBg)}" alt="" style="display:block">` : ""}<div class="header-content"><h1>${escapeHtml(siteTitle || '旭儿导航')}</h1>${cnLink ? `<a href="${escapeHtml(cnLink)}" class="cn-btn" target="_blank">🇨🇳 国内线路</a>` : ''}<p>${escapeHtml(siteSubtitle || '精选网站 · 优质博客')}</p><div>📅 ${new Date().toLocaleDateString('zh-CN')}</div></div></div>
        <div class="content">
